...
#ifndef postAnalyzer_ZnunuG_h
#define postAnalyzer_ZnunuG_h
...
public :
   ...
   // 8 selt bins: ""  "_sbUp" "_sbDown" "_metUp" "_metDown" "_binUp" "_binDown" "_noPiso"
   std::vector<int> ptbins;          //6
   std::vector<TString> ptbinnames;  //7 = (6-1)+1+1
   std::vector<TString> selbinnames; //0
   ...
   TH1F h_sig_et[7][8], 
        ...
        h_sig_pfMET[7][8],
        h_sig_leptoMET[7][8],
        h_sig_dilep_mass[7][8],
        h_sig_diele_mass[7][8],
        h_sig_dimu_mass[7][8];
   ...
   TH1F h_gen_et[7][8], 
        ...
        h_gen_pfMET[7][8],
        h_gen_leptoMET[7][8],
        h_gen_dilep_mass[7][8],
        h_gen_diele_mass[7][8],
        h_gen_dimu_mass[7][8];
   ...
   bool passGenMET110;
   bool passGenMET170;
   bool passGendPhiPhoMET;
   bool passGenZWindow;

  // counters
  int nrec_m110_ywnd_ydphi;
  int nrec_m110_ywnd_ndphi;
  int nrec_m110_nwnd_ydphi;
  int nrec_m110_nwnd_ndphi;
  int nrec_m170_ywnd_ydphi;
  int nrec_m170_ywnd_ndphi;
  int nrec_m170_nwnd_ydphi;
  int nrec_m170_nwnd_ndphi;

  int ngen_m110_ywnd_ydphi;
  int ngen_m110_ywnd_ndphi;
  int ngen_m110_nwnd_ydphi;
  int ngen_m110_nwnd_ndphi;
  int ngen_m170_ywnd_ydphi;
  int ngen_m170_ywnd_ndphi;
  int ngen_m170_nwnd_ydphi;
  int ngen_m170_nwnd_ndphi;
   ...
   postAnalyzer_ZnunuG(TTree *tree=0);
   virtual ~postAnalyzer_ZnunuG();
...
#ifdef postAnalyzer_ZnunuG_cxx
postAnalyzer_ZnunuG::postAnalyzer_ZnunuG(TTree *tree) : fChain(0) 
{
}

postAnalyzer_ZnunuG::~postAnalyzer_ZnunuG()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t postAnalyzer_ZnunuG::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t postAnalyzer_ZnunuG::LoadTree(Long64_t entry)
{
...
void postAnalyzer_ZnunuG::Init(TTree *tree, Bool_t isMC)
{
   ...
   selbinnames.push_back("_m110_ywnd_ydphi"); 
   selbinnames.push_back("_m110_ywnd_ndphi"); 
   selbinnames.push_back("_m110_nwnd_ydphi"); 
   selbinnames.push_back("_m110_nwnd_ndphi"); 
   selbinnames.push_back("_m170_ywnd_ydphi"); 
   selbinnames.push_back("_m170_ywnd_ndphi"); 
   selbinnames.push_back("_m170_nwnd_ydphi"); 
   selbinnames.push_back("_m170_nwnd_ndphi"); 
...
//     // set up names
     ...
     TString histname_gen_pfMET = "h_gen_pfMET_"+ptbinnames[i]+selbinnames[j];
     TString histname_gen_leptoMET = "h_gen_leptoMET_"+ptbinnames[i]+selbinnames[j];
     TString histname_gen_dilep_mass = "h_gen_dilep_mass_"+ptbinnames[i]+selbinnames[j];
     TString histname_gen_diele_mass = "h_gen_diele_mass_"+ptbinnames[i]+selbinnames[j];
     TString histname_gen_dimu_mass = "h_gen_dimu_mass_"+ptbinnames[i]+selbinnames[j];
     ...
     h_sig_leptoMET[i][j] = TH1F(histname_sig_leptoMET,"PF MET + dilepton",300,0.,300.);
     h_sig_leptoMET[i][j].Sumw2();
     //
     h_sig_dilep_mass[i][j].Clear();
     h_sig_dilep_mass[i][j] = TH1F(histname_sig_dilep_mass,"mass dilepton (MET)",30,60.,120.);
     h_sig_dilep_mass[i][j].Sumw2();
     //
     h_sig_dimu_mass[i][j].Clear();
     h_sig_dimu_mass[i][j] = TH1F(histname_sig_dimu_mass,"mass dimuon (MET)",30,60.,120.);
     h_sig_dimu_mass[i][j].Sumw2();
     //
     h_sig_diele_mass[i][j].Clear();
     h_sig_diele_mass[i][j] = TH1F(histname_sig_diele_mass,"mass diele (MET)",30,60.,120.);
     h_sig_diele_mass[i][j].Sumw2();


     h_gen_et[i][j].Clear();
     h_gen_et[i][j] = TH1F(histname_gen_et,"Photon Transverse Energy",165,175.,1000.);
     h_gen_et[i][j].Sumw2();
     //
     h_gen_eta[i][j].Clear();
     h_gen_eta[i][j] = TH1F(histname_gen_eta,"Leading Photon Eta",100,-2.,2.);
     h_gen_eta[i][j].Sumw2();
     //
     h_gen_pfMET[i][j].Clear();
     h_gen_pfMET[i][j] = TH1F(histname_gen_pfMET,"ParticleFlow MET",300,0.,300.);
     h_gen_pfMET[i][j].Sumw2();
     //
     h_gen_leptoMET[i][j].Clear();
     h_gen_leptoMET[i][j] = TH1F(histname_gen_leptoMET,"PF MET + dilepton",300,0.,300.);
     h_gen_leptoMET[i][j].Sumw2();
     //
     h_gen_dilep_mass[i][j].Clear();
     h_gen_dilep_mass[i][j] = TH1F(histname_gen_dilep_mass,"mass dilepton (MET)",30,60.,120.);
     h_gen_dilep_mass[i][j].Sumw2();
     //
     h_gen_dimu_mass[i][j].Clear();
     h_gen_dimu_mass[i][j] = TH1F(histname_gen_dimu_mass,"mass dimuon (MET)",30,60.,120.);
     h_gen_dimu_mass[i][j].Sumw2();
     //
     h_gen_diele_mass[i][j].Clear();
     h_gen_diele_mass[i][j] = TH1F(histname_gen_diele_mass,"mass diele (MET)",30,60.,120.);
     h_gen_diele_mass[i][j].Sumw2();
...
Bool_t postAnalyzer_ZnunuG::Notify()
{
...
void postAnalyzer_ZnunuG::Show(Long64_t entry)
{
...
Int_t postAnalyzer_ZnunuG::Cut(Long64_t entry)
{
...
Bool_t postAnalyzer_ZnunuG::FillGenHistograms(int ptbin, int selbin, int photonIndex, double weight){
 weight = 1;
 h_gen_et[ptbin][selbin].Fill( mcEt->at(photonIndex), weight );
 h_gen_eta[ptbin][selbin].Fill( mcEta->at(photonIndex), weight );
 h_gen_pfMET[ptbin][selbin].Fill( genMET, weight );
 h_gen_leptoMET[ptbin][selbin].Fill( genLeptoMET, weight );
 h_gen_dilep_mass[ptbin][selbin].Fill( gen_dilep_mass, weight );
 h_gen_dimu_mass[ptbin][selbin].Fill( fourVec_genmm.M(), weight );
 h_gen_diele_mass[ptbin][selbin].Fill( fourVec_genee.M(), weight );
...
#endif // #ifdef postAnalyzer_ZnunuG_cxx
