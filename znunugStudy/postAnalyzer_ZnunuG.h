...
//     // set up names
     ...
     //TString histname_sig_leptoMET = "h_sig_leptoMET_"+ptbinnames[i]+sysbinnames[j];
     ...
     TString histname_sig_leptoMET = "h_sig_leptoMET_"+ptbinnames[i];
     TString histname_sig_dilep_mass = "h_sig_dilep_mass_"+ptbinnames[i];
     TString histname_sig_diele_mass = "h_sig_diele_mass_"+ptbinnames[i];
     TString histname_sig_dimu_mass = "h_sig_dimu_mass_"+ptbinnames[i];

     TString histname_gen_et  = "h_gen_et_"+ptbinnames[i];
     TString histname_gen_eta = "h_gen_eta_"+ptbinnames[i];
     TString histname_gen_pfMET = "h_gen_pfMET_"+ptbinnames[i];
     TString histname_gen_leptoMET = "h_gen_leptoMET_"+ptbinnames[i];
     TString histname_gen_dilep_mass = "h_gen_dilep_mass_"+ptbinnames[i];
     TString histname_gen_diele_mass = "h_gen_diele_mass_"+ptbinnames[i];
     TString histname_gen_dimu_mass = "h_gen_dimu_mass_"+ptbinnames[i];
     ...
     h_sig_pfMET[i].Sumw2();
     //
     h_sig_leptoMET[i].Clear();
     h_sig_leptoMET[i] = TH1F(histname_sig_leptoMET,"PF MET + dilepton",300,0.,300.);
     h_sig_leptoMET[i].Sumw2();
     //
     h_sig_dilep_mass[i].Clear();
     h_sig_dilep_mass[i] = TH1F(histname_sig_dilep_mass,"mass dilepton (MET)",30,60.,120.);
     h_sig_dilep_mass[i].Sumw2();
     //
     h_sig_dimu_mass[i].Clear();
     h_sig_dimu_mass[i] = TH1F(histname_sig_dimu_mass,"mass dimuon (MET)",30,60.,120.);
     h_sig_dimu_mass[i].Sumw2();
     //
     h_sig_diele_mass[i].Clear();
     h_sig_diele_mass[i] = TH1F(histname_sig_diele_mass,"mass diele (MET)",30,60.,120.);
     h_sig_diele_mass[i].Sumw2();
     ...
     //
     h_gen_eta[i].Clear();
     h_gen_eta[i] = TH1F(histname_gen_eta,"Leading Photon Eta",100,-2.,2.);
     h_gen_eta[i].Sumw2();
     //
     h_gen_pfMET[i].Clear();
     h_gen_pfMET[i] = TH1F(histname_gen_pfMET,"ParticleFlow MET",300,0.,300.);
     h_gen_pfMET[i].Sumw2();
     //
     h_gen_leptoMET[i].Clear();
     h_gen_leptoMET[i] = TH1F(histname_gen_leptoMET,"PF MET + dilepton",300,0.,300.);
     h_gen_leptoMET[i].Sumw2();
     //
     h_gen_dilep_mass[i].Clear();
     h_gen_dilep_mass[i] = TH1F(histname_gen_dilep_mass,"mass dilepton (MET)",30,60.,120.);
     h_gen_dilep_mass[i].Sumw2();
     //
     h_gen_dimu_mass[i].Clear();
     h_gen_dimu_mass[i] = TH1F(histname_gen_dimu_mass,"mass dimuon (MET)",30,60.,120.);
     h_gen_dimu_mass[i].Sumw2();
     //
     h_gen_diele_mass[i].Clear();
     h_gen_diele_mass[i] = TH1F(histname_gen_diele_mass,"mass diele (MET)",30,60.,120.);
     h_gen_diele_mass[i].Sumw2();
...
Bool_t postAnalyzer_ZnunuG::FillGenHistograms(int ptbin, int sysbin, int photonIndex, double weight){
 weight = 1;
 ...
 h_gen_leptoMET[ptbin].Fill( genLeptoMET, weight );
 h_gen_dilep_mass[ptbin].Fill( gen_dilep_mass, weight );
 h_gen_dimu_mass[ptbin].Fill( fourVec_genee.M(), weight );
 h_gen_diele_mass[ptbin].Fill( fourVec_genmm.M(), weight );

 return kTRUE;
}

Bool_t postAnalyzer_ZnunuG::FillSigHistograms(int ptbin, int sysbin, int photonIndex, double weight){
 ...
 h_sig_leptoMET[ptbin].Fill( leptoMET, weight );
 h_sig_dilep_mass[ptbin].Fill( dilep_mass, weight );
 h_sig_dimu_mass[ptbin].Fill( fourVec_ee.M(), weight );
 h_sig_diele_mass[ptbin].Fill( fourVec_mm.M(), weight );

 return kTRUE;
}

Bool_t postAnalyzer_ZnunuG::WriteHistograms(int ptbin, int sysbin){
 h_sig_et[ptbin].Write();
 h_sig_eta[ptbin].Write();
 h_sig_sieieF5x5[ptbin].Write();
 h_sig_pfMET[ptbin].Write();
 h_sig_leptoMET[ptbin].Write();  
 h_sig_dilep_mass[ptbin].Write();
 h_sig_dimu_mass[ptbin].Write(); 
 h_sig_diele_mass[ptbin].Write();

 h_gen_et[ptbin].Write();
 h_gen_eta[ptbin].Write();
 h_gen_leptoMET[ptbin].Write();  
 h_gen_dilep_mass[ptbin].Write();
 h_gen_dimu_mass[ptbin].Write(); 
 h_gen_diele_mass[ptbin].Write();
