...
void postAnalyzer_znunug::Loop(TString outfilename, Bool_t isMC, Double_t lumi, Double_t nrEvents, Double_t crossSec)
{
 ...
 //for (Long64_t jentry=0; jentry<1000;jentry++) {
  ...
  {   
   ...
   //}
   phoCand[0] = getPhoCand(175,1.4442);
   if(phoCand[0].size()>0)
     {
      int candphotonindex = phoCand[0].at(0);
      ...
      bool passdphiJetMET(jetindexvector)
      ...
      if ( passSpike && passMETfilters && passdPhiJetMET && passMET && passdPhiPhoMET )
         {
          for(unsigned int ptb=0; ptb<lastptbin-2; ++ptb){ // break into pT bins
           if(
            ...
            FillSigHistograms(ptb, sysb, candphotonindex, event_weight);
          ...
          if(  // do an inclusive pT plot from bins
           ...
           FillSigHistograms(lastptbin-1, sysb, candphotonindex, event_weight);
          }
          // and one fully inclusive in pT
          FillSigHistograms(lastptbin, sysb, candphotonindex, event_weight);
         }
      // end fill histograms
...
//-------------------------selectedJets
std::vector<int> postAnalyzer::selectedJets(int pho_index) {
...
//-------------------------passdphiJetMET
bool postAnalyzer::passdphiJetMET(std::vector<int> jets, double mephi)
{
...
//-------------------------makeDilep
void postAnalyzer::makeDilep(int pho_index, TLorentzVector *fv_1, TLorentzVector *fv_2)
{
  ...
  // muons
  if( mulist.size()>1 ){
   m1.SetPtEtaPhiE( muPt[mulist[0]], muEta[mulist[0]], muPhi[mulist[0]], muEn[mulist[0]] );
   for(int i=1; i<mulist.size(); ++i)
   {
     if( muCharge[0]*muCharge[i]==-1 )
     {
      m2.SetPtEtaPhiE( muPt[mulist[i]], muEta[mulist[i]], muPhi[mulist[i]], muEn[mulist[i]] );
      break;
     }
   }
   mm = m1 + m2;
  ...
  if( ee.M() > mm.M() && ee.M()>60. && ee.M()<120. ){ fv_1 = e1; fv2 = e2; }
  if( ee.M() < mm.M() && mm.M()>60. && mm.M()<120. ){ fv_1 = m1; fv2 = m2; }
...
std::vector<int> postAnalyzer::electron_passLooseID(int pho_index, float elePtCut)
{
...
//-------------------------muon_passLooseID
std::vector<int> postAnalyzer::muon_passLooseID(int pho_index, float muPtCut)
{
