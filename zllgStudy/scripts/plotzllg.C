...
void plotzllg::Loop()
{
 ...
 variablenames.push_back("mu_et"        ); 
 variablenames.push_back("mu_uncorret"  );
 variablenames.push_back("ele_et"        ); 
 variablenames.push_back("ele_uncorret"  );
...
// selectionnames.push_back("m110");
 ...
 Int_t rebin = rebins[v];
  ...
  TString selectionname = selectionnames[s];
   ...
   h_TGJets  ->SetFillColor( kAzure-9  ); 
   ...
   h_EFake   ->Scale( h_EFake->Integral()/10. );
   ...
   h_GJets->SetXTitle(TString(data_obs->GetTitle())+" [GeV]");
   h_GJets->SetTitle("");
   h_GJets->SetYTitle("Events / Bin");
   h_GJets->GetYaxis()->SetTitleOffset(1.4);
